...
3. `dotnet restore` zonder aparte cache-actie in v1: bij een handvol packages duurt een schone restore op `ubuntu-latest` seconden. De ingebouwde NuGet-cache van setup-dotnet vereist `packages.lock.json`-bestanden plus locked-mode restore en is daarmee te veel complexiteit voor de winst — dat is doorgroei (zie §6).
...
- **Caching**: geen aparte package- of layer-caching in v1. Docker layer-caching is uitgesloten omdat er zonder registry geen goedkope cache-backend is.
...
Zodra publiceren nodig is: `docker`-job uitbreiden met registry-login + `push: true` (alleen op `push` naar `main`/tags, niet op PRs), layer-caching aanzetten, en desgewenst Trivy-scan en Codecov toevoegen. NuGet-caching kan later via `packages.lock.json`-bestanden + `cache: true` in setup-dotnet (let op: zonder lock-files faalt die optie). Dit volgt het pad van aanpak C uit de brainstorm.
